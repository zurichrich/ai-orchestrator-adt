...
|---|---|---|---|
| `hooks/adt-phrase-linter.sh` | `Stop` | Parses the last assistant turn from the transcript; flags banned recovery-narration phrases, an unbacked done-claim about a rendered artifact, and an unbacked **pass-claim** (ADT-114 ENFORCED RULE 1 — a claim that something passed with no matching command run this turn). Enforces working-style mechanically. | no (warn) |
