...
Stop your agents writing the wrong thing and over complicatng your requests.
Run mulitple agents in parallel on the same project without compromising quality.
...
```bash
git -C ~/agent-dev-team pull && ~/agent-dev-team/adt-install.sh
```
...
```bash
~/agent-dev-team/adt-install.sh --uninstall --everyone
```
...
your-repo/                     # CODE ONLY — no backlog tree
  .claude/                     # rules/skills/agents/hooks/commands/tools copied from ~/agent-dev-team,
...
~/agent-dev-team/              # ADT itself — playbooks, roles, defaults, tools
...
```bash
git clone --branch v0.2.0 --depth 1 https://github.com/zurichrich/ai-orchestrator-adt.git ~/agent-dev-team
```
